...
LIST_ITEMTYPE list_pop_idx (list_t** l, unsigned long c) {
  LIST_ITEMTYPE p = 0;
  if (!l || list_empty((*l)) || c >= (*l)->length)
    return (0);
  if (c == (*l)->length-1)
...
unsigned long list_lookup (list_t* l, int (*cmp) (const LIST_ITEMTYPE, const LIST_ITEMTYPE), const LIST_ITEMTYPE p) {
  unsigned long i = 0;
  if (list_empty(l) || !*cmp)
    return (-1);
  for (i = 0; i < l->length; i++)
